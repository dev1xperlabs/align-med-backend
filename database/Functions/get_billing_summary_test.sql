-- FUNCTION: public.get_billing_summary_test()

-- DROP FUNCTION IF EXISTS public.get_billing_summary_test();

CREATE OR REPLACE FUNCTION public.get_billing_summary_test(
	)
...
BEGIN
    -- Today & Yesterday
    SELECT
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE = CURRENT_DATE), 0),
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE = CURRENT_DATE - INTERVAL '1 day'), 0)
    INTO today_total, yesterday_total
    FROM public.bills;

    -- This Week & Last Week
    SELECT
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE >= week_start AND billed_date::DATE <= CURRENT_DATE), 0),
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE >= last_week_start AND billed_date::DATE <= last_week_end), 0)
    INTO this_week_total, last_week_total
    FROM public.bills;

    -- This Month & Last Month
    SELECT
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE >= month_start AND billed_date::DATE <= CURRENT_DATE), 0),
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE >= last_month_start AND billed_date::DATE <= last_month_end), 0)
    INTO this_month_total, last_month_total
    FROM public.bills;

    -- This Year & Last Year
    SELECT
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE >= year_start AND billed_date::DATE <= CURRENT_DATE), 0),
        COALESCE(SUM(total_billed_charges) FILTER (WHERE billed_date::DATE >= last_year_start AND billed_date::DATE <= last_year_end), 0)
    INTO this_year_total, last_year_total
    FROM public.bills;

    -- Percentage Changes (rounded)
...
ALTER FUNCTION public.get_billing_summary_test()
    OWNER TO postgres;
